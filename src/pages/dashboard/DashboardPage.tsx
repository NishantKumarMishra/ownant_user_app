...
// ── Metric Card ───────────────────────────────────────────────
function MetricCard({
  label, value, hint, icon, danger = false,
}: {
  label: string
  value: string
  hint?: string
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center justify-between mb-2">
        <p className={cn('text-xs font-medium', danger ? 'text-danger/70' : 'text-textSecondary')}>
          {label}
        </p>
        <span className={cn('opacity-40', danger ? 'text-danger' : 'text-textSecondary')}>
          {icon}
        </span>
      </div>
      <p className={cn(
      ...
      {hint && (
        <p className={cn('text-xs mt-1', danger ? 'text-danger/60' : 'text-textSecondary')}>
          {hint}
        </p>
      )}
    </div>
...
function ActivityItem({ item }: { item: ActivityFeedItem }) {
  const inner = (
    <div className="flex items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-3 hover:border-primary/20 transition-colors">
      <div className={cn('h-2 w-2 rounded-full flex-shrink-0', DOT_COLORS[item.dotColor])} />
    ...
    <div className="space-y-5 pb-24">
      ...
      {overdue > 0 && (
        ...
        >
          <p className="text-sm font-semibold text-danger">
            {overdue} payment{overdue > 1 ? 's' : ''} overdue — collect now
          </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <MetricCard
          ...
          hint={`${totalBeds - occupiedBeds} available`}
          icon={<BedDouble className="h-4 w-4" />}
        />
        <MetricCard
          label="Overdue"
          value={String(overdue)}
          hint={overdue > 0 ? 'Need collection' : 'All clear ✓'}
      ...
      {upcomingDues.length > 0 && (
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-textPrimary">Upcoming dues</h2>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-end justify-between">
                    <div>
                      <p className="text-base font-bold text-textPrimary">{fmtINR(p.amountDue)}</p>
                      <p className="text-xs text-textSecondary mt-0.5">{dueLabel}</p>
                    </div>
      ...
      {/* ── Quick actions ──────────────────────────────────── */}
      <section>
        <h2 className="text-sm font-semibold text-textPrimary mb-3">Quick actions</h2>
        <div className="grid grid-cols-4 gap-3">
          ...
          >
            ...
            <span className="text-[10px] font-medium text-textSecondary text-center leading-tight">
              {reminders.isPending ? 'Sending…' : `Send\nReminder`}
            </span>
          </button>
        ...
        <section>
          ...
          {activityFeed.length > 4 && (
            ...
            >
              {showAllActivity
                ? 'Show less ↑'
                : `See all ${activityFeed.length} activities ↓`}
            </button>
          )}
        </section>
      )}
      ...
      {trend.length > 0 && (
        <section>
          <div className="rounded-2xl border border-border bg-surface p-4">
            <h2 className="text-sm font-semibold text-textPrimary">6-month collection</h2>
            <p className="text-xs text-textSecondary mt-0.5">Expected vs collected</p>
            <div className="mt-4">
