...
export function ProfilePage() {
  ...
  const barColor =
    bedPct > 95 ? "bg-danger" : bedPct > 80 ? "bg-warning" : "bg-success";
      ...
      {isLoading || !owner ? (
        ...
        <>
          ...
          <div className="flex items-center gap-4 bg-surface rounded-2xl border border-border p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-textMuted mt-0.5">
                Member since{" "}
                {new Date(owner.memberSince).toLocaleDateString("en-IN", {
                  month: "long",
                  year: "numeric",
                })}
              </p>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      },
                      {
                        ...
                        label: "Enquiries",
                        value: listing.enquiriesCount,
                      },
                      {
                        icon: <TrendingUp className="h-4 w-4" />,
                        label: "Pending",
                        value: listing.pendingEnquiries,
                      },
                    ].map((s) => (
                      <div
                        key={s.label}
                        className="bg-bg rounded-xl border border-border p-3 text-center"
                      >
          ...
          <div className="bg-surface rounded-2xl border border-border overflow-hidden divide-y divide-border">
            ...
            <button
              disabled
              className="px-4 py-4 text-left text-sm text-textMuted w-full"
            >
              Notification settings (coming soon)
            </button>
            <a
              href="mailto:support@ownant.com"
              className="flex items-center justify-between px-4 py-4 text-sm hover:bg-bg transition-colors"
            >
              Help & Support
              <ChevronRight className="h-4 w-4 text-textSecondary" />
            </a>
